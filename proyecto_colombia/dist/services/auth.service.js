...
let AuthService = class AuthService {
    ...
    validarTokenJWT(token) {
        try {
            let datos = jwt.verify(token, config_1.config.claveJWT); // paso donde verifica el "token" generado por el usuario inscrito con respecto a la clave general JWT "config.claveJWT" del proyecto
            return datos;
        }
        catch (error) {
            return false;
